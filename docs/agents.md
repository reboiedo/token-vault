...
| Door | When | How |
|---|---|---|
| **MCP tools** (preferred) | Interactive agent sessions | `npx token-vault mcp -d design-system` (stdio). 14 tools, listed below. |
...
```bash
npx token-vault init  [-d dir] [-n name]   # scaffold
npx token-vault dev   [-d dir] [-p port]   # editor + RPC + watcher
npx token-vault check [-d dir]             # validate (run after EVERY edit session)
npx token-vault build [-d dir] [-o out]    # DTCG → dist/
