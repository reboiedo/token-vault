...
export async function startMcpServer(dir: string): Promise<void> {
  ...
  const INSTRUCTIONS = [
      ...
      "",
      "system.json surfaceRecipes ('off'|'css'|'dtcg'|'both') also exports the",
      "surfaces rules as seed-driven relative colors (color-mix/oklch(from ...))",
      "driven by --surface/--ink, on top of the baked per-surface tokens. APCA",
      "levels ship an approximate recipe (flagged approx); the bakes stay exact.",
      "",
