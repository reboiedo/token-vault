...
editor is `npx token-vault-studio dev` on localhost, and **git is history and
...
3. **Verify after editing.** `npx token-vault-studio check -d <dir>` catches
...
- `dist/` is build output (`npx token-vault-studio build`) — never edit it.
